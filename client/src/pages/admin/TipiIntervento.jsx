...
export default function TipiIntervento() {
  ...
  async function load() {
    try {
      const data = await apiFetch("/tipi-intervento");
      setTipi(data);
    } catch (err) {
  ...
  async function add(e) {
    ...
    setLoading(true);
    try {
      await apiFetch("/tipi-intervento", {
        method: "POST",
  ...
  async function remove(id) {
    setError("");
    try {
      await apiFetch(`/tipi-intervento/${id}`, { method: "DELETE" });
      await load();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <section>
      ...
      <p className="info" style={{ marginBottom: "20px" }}>
        Gestisci qui la lista globale dei tipi di intervento. I template che includono
        il campo <strong>"Tipologia di intervento"</strong> useranno automaticamente questi valori.
      </p>
      ...
      <div className="card" style={{ maxWidth: "560px" }}>
        <form onSubmit={add} style={{ display: "flex", gap: "10px", marginBottom: "24px" }}>
          <input
            ref={inputRef}
            placeholder="Nuovo tipo di intervento (es. Potatura)"
        ...
        {tipi.length === 0 ? (
          <p style={{ color: "#666" }}>
            Nessun tipo di intervento configurato. Aggiungine uno sopra.
          </p>
        ) : (
