...
router.get("/", authRequired, async (_req, res) => {
  try {
    const rows = await all(
      "SELECT id, nome, tipo, attivo, is_default, created_at FROM campi_template ORDER BY is_default DESC, id ASC"
    );
...
router.post("/", authRequired, requireRole("admin"), async (req, res) => {
  try {
    ...
    const result = await run(
      "INSERT INTO campi_template (nome, tipo, attivo, is_default) VALUES (?, ?, 0, 0)",
      [nome, tipo]
    );

    const created = await get("SELECT id, nome, tipo, attivo, is_default, created_at FROM campi_template WHERE id = ?", [result.lastID]);
...
router.patch("/:id", authRequired, requireRole("admin"), async (req, res) => {
  try {
    ...
    await run("UPDATE campi_template SET attivo = ? WHERE id = ?", [attivo, req.params.id]);

    const updated = await get("SELECT id, nome, tipo, attivo, is_default, created_at FROM campi_template WHERE id = ?", [req.params.id]);
